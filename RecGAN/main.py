    ...
    while not stop_training and epoch <= n_epochs:
        ...
        if eval_type == 'valid' and epoch <= n_epochs:
            if eval_acc > inner_val_acc_best or eval_map > inner_val_map_best:
                ...
                if not os.path.exists(outputdir):
                    os.makedirs(outputdir)
                torch.save(model.state_dict(), os.path.join(outputdir, 'seqGan_dis.' + outputmodelname))
                inner_val_acc_best = eval_acc
...
if __name__ == '__main__':
    ...
    #Routes
    pretrained_gen =  'model_output/seqGan_gen3.simu.model.pth'
    pretrained_dis =  'model_output/seqGan_dis.simu.model.pth'
    pretrained_agent = 'model_output/seqGan_agent3.simu.model.pth'
    final_dis = 'model_output/seqGan_dis1.model.simu.pth'
